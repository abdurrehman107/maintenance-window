...
func (v *DeploymentCustomValidator) ValidateCreate(ctx context.Context, obj runtime.Object) (admission.Warnings, error) {
	...
	// make sure that the deployment is not in a maintenance window
	// get all the maintenance windows 
	maintenanceWindow := maintenancecustomiov1.MaintenanceWindowList{}
	if err := v.Client.List(ctx, &maintenanceWindow); err != nil {
		return nil, err
	}
	for _, obj := range maintenanceWindow.Items {
		if obj.Status.Active == true{
			return nil, fmt.Errorf("maintenance window is active")
		}
	}
