...
import (
	...
	test_utils "github.com/abdurrehman107/maintenance-window/test/utils"
)

var (
	testEnv     env.Environment
	dockerImage = "maintenance-window:v0.1"
	namespace   = "maintenance-window-system"
)

func TestMain(m *testing.M) {
	testEnv = env.New()
	kindClusterName := "mw-test-cluster"
	...
	testEnv.Setup(
		...
		func(ctx context.Context, cfg *envconf.Config) (context.Context, error) {
			...
			if err := wait.For(
				conditions.New(client.Resources()).DeploymentAvailable("maintenance-window-controller-manager", namespace),
				wait.WithTimeout(3*time.Minute),
				wait.WithInterval(10*time.Second),
			); err != nil {
				log.Printf("Timed out while waiting for mw-controller-manager deployment: %s", err)
				return ctx, err
			}
